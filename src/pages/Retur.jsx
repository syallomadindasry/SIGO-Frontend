// File: frontend/src/pages/Retur.jsx
...
export default function Retur({ user, stokData, reloadStok }) {
  ...
  async function load() {
    setBusy(true);
    try {
      setList(await api.getRetur(user.id_gudang));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function loadDetail(id) {
    setBusy(true);
    try {
      setDetail(await api.getReturDetail(id));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function saveMaster() {
    ...
    setBusy(true);
    try {
      const res = await api.saveReturMaster({
        ...masterForm,
        id_admin: user.id_admin,
        id_gudang: user.id_gudang,
        tujuan: 1, // default tujuan dinkes (sesuaikan jika id gudang dinkes berbeda)
      });

      setOpenMaster(false);
      await load();

      const created = { ...masterForm, id: res.id, tujuan: 1 };
      setSelected(created);
  ...
  async function addDetail() {
    ...
    setBusy(true);
    try {
      await api.saveReturDetail({
        id_retur: selected.id,
  ...
  function printNota() {
    ...
    openPrintWindow(
      "Nota Retur",
      `
        <h2>NOTA RETUR (ke Dinkes)</h2>
        <div class="muted">SIGO – Sistem Informasi Gudang Obat</div>
        <hr/>
        <div><b>No:</b> ${selected.no_retur}</div>
    ...
    <div>
      <div className="card">
        <div className="card-header">
          <div className="card-title">↩️ Retur ke Dinkes</div>
          <div className="row">
            <button className="btn btn-warning" onClick={() => setOpenMaster(true)}>
              ➕ Buat Retur
            </button>
            ...
            <tbody>
              {list.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>{r.alasan || "-"}</td>
                  <td>
                    <span className="badge badge-warning">{r.total_item} item</span>
                  </td>
                  <td>
                    <button
      ...
      {selected ? (
        <div className="card">
          <div className="card-header">
            <div className="card-title">📦 Detail: {selected.no_retur}</div>
            <div className="row">
              <button className="btn btn-warning" onClick={() => setOpenDetail(true)}>
                ➕ Tambah Item
              </button>
              ...
              <tbody>
                {detail.map((d, i) => (
                  <tr key={i}>
                    ...
                    <td>
                      <span className="badge badge-warning">{d.batch}</span>
                    </td>
                    <td>
      ...
      {openMaster ? (
        <Modal
          ...
          footer={
            <>
              ...
              <button className="btn btn-warning" onClick={saveMaster} disabled={busy}>
                Simpan
              </button>
            </>
          }
        >
          <div className="form-grid">
            <div>
              <div className="label">No Retur</div>
              <input className="input" value={masterForm.no_retur} onChange={(e) => setMasterForm({ ...masterForm, no_retur: e.target.value })} />
            </div>
            <div>
        ...
        <Modal
          ...
          footer={
            <>
              ...
              <button className="btn btn-warning" onClick={addDetail} disabled={busy}>
                Tambah
              </button>
            </>
          }
